...
static uint8_t timestwo(uint8_t a)
{
	uint8_t reducpoly = 0x1b; //0b100011011 or 0x11b. never will use 2^8 though...

	//if(x == 0x39)
	//	printf("\nComputing the sum....\n\n");

	uint8_t seven = get_bit(a, 7);
	uint8_t sum = 0x00;
	for(int y = 0; y < 7; y++)
	{
		//sum = set_bit(sum, (y+1), ( (seven*get_bit(reducpoly, (y+1))) + get_bit(a, y) ));
		
		sum += (seven*(get_bit(reducpoly, (y+1))) + get_bit(a, y));

		//if(x == 0x39)
		//	printf("Sum: 0x%hhx. A7: 0x0%hhx. Mi-1: 0x0%hhx. Ai: 0x0%hhx\n\n", sum, seven, get_bit(reducpoly, (y+1)), get_bit(a, y));
	}
	//if(x == 0x39)
	//	printf("\nKnown input: 0x%hhx. A7 + Sum: 0x0%hhx. Predicted Ham code: 0x0%hhx\n", a, (seven + sum), hamming_encode(seven + sum));
	//printf("Sum:%hhx ", sum);
...
static void hamtransform()
{
	//timestwo();
	//uint16_t reducpoly = 0x11b;
	
	for(int x = 0; x < 256; x++)
	{
		//uint16_t mult = (( (uint16_t)(sbox[x]) << 1) % reducpoly);
		//hamming_encode(timestwo(sbox[x], x));
		//printf("0x%hhx, ", timestwo(sbox[x]) );
		printf("0x0%hhx, ", hamming_encode(timestwo(sbox[x])) );
		if(((x+1) % 16) == 0)
			printf("\n");
	}
	...
	{
		//uint16_t mult = (( (uint16_t)(sbox[x]) << 1) % reducpoly);
		//hamming_encode(Multiply(sbox[x], 0x02));
		printf("0x0%hhx, ", hamming_encode(Multiply(sbox[x], 0x03)));
		if(((x+1) % 16) == 0)
			printf("\n");
	}
	printf("\n\n");
