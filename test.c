...
{
	uint16_t reducpoly = 0x11b; //0b100011011
	h_2rd[0] = 0x69;
	//printf("Original h_2rd:\n");
	for(int x = 0; x < 256; x++)
	{
		uint8_t seven = get_bit(h_2rd[x], 7);
		uint8_t sum = 0x00;
		for(int y = 0; y < 7; y++)
		{
			sum += (( seven*get_bit(reducpoly, (y+1)) ) + get_bit(h_2rd[x], y));
		}
		//printf("Sum:%hhx ", sum);
		h_2rd[x] = seven + sum;
		//printf("0x0%hhx, ", h_2rd[x]);
	}
	//printf("\n");
}
